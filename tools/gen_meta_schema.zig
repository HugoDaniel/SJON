//! Generator for `src/MetaSchema.generated.zig`.
//!
//! `manifests/meta.sjon` is the single source of truth for the bootstrap
//! meta-plugin. This tool loads it via `ManifestLoader.loadUnchecked`
//! (ordinary `load` would be circular — it meta-validates against the
//! very schema we are generating) and emits the resulting `Plugin` as a
//! comptime Zig literal.
//!
//! The emitted text is run through Zig's own formatter (`Ast.render`)
//! before it is written or compared, so the output is canonical and
//! byte-stable across runs regardless of this file's spacing choices.
//!
//! Run modes (mirrors `examples/export-schema-demo.zig`):
//!   - default: compare the freshly-generated bytes against the
//!     checked-in `src/MetaSchema.generated.zig`; exit 1 on any diff.
//!     `zig build test` invokes this, so editing `meta.sjon` without
//!     regenerating (or hand-editing the generated file) fails CI.
//!   - `--regen`: overwrite `src/MetaSchema.generated.zig` so a human can
//!     review the diff and re-commit. Run after an intentional change to
//!     `meta.sjon`.
//!
//! Coverage is intentionally narrow: the emitter handles exactly the
//! `Plugin` shapes `meta.sjon` uses today and `@panic`s on anything else
//! (a discriminated meta-form, an expr-func, a `:default`, …). A loud
//! abort beats silently dropping a field as the meta-plugin grows.
...
pub fn main(init: std.process.Init) !u8 {
    ...
    // --- emit rough Zig, then canonicalise via the formatter ---
...
/// Emit the plugin as rough Zig, parse it, and render it through the
/// canonical formatter. Returns owned bytes (caller frees).
fn renderPlugin(gpa: Allocator, plugin: Plugin.Plugin) ![]u8 {
    ...
    if (ast.errors.len != 0) {
        // The emitter produced invalid Zig — a generator bug, not user
        // input. Fail loudly so it is fixed rather than committed.
...
// ---------------------------------------------------------------------------
// Emitters — write rough Zig; the formatter canonicalises spacing.
// ---------------------------------------------------------------------------
...
fn emitValueKind(w: *Writer, vk: Plugin.ValueKind) Writer.Error!void {
    // Refinements the meta-plugin never uses: bail loudly if one appears.
...
fn emitForm(w: *Writer, f: Plugin.FormSpec) Writer.Error!void {
    // Form features the meta-plugin never uses: bail loudly if one appears.
