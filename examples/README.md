...
| [`plugins/xref/`](plugins/xref) | Every `(cross-ref …)` axis in one manifest — bare, `:acyclic`, `:scope`-bound, and the provider route (`:provider` + `:source-key`, whose member set is extracted from a string in the document). Read it beside the four export goldens: the routes differ in what an exporter can say about them, not in the JSON they produce. |
