...
fn run_inline_case(name: &str) {
    run_inline_case_with(name, None);
}

/// `run_inline_case` for a case whose family declares a validator option.
/// `held_symbol` is `classifier.json`'s `validatorOptionFamilies` value,
/// resolved at build time and baked into the generated call — the two
/// spellings of one case cannot drift because only one of them is emitted.
fn run_inline_case_with(name: &str, held_symbol: Option<&str>) {
    let case_dir = corpus_dir().join(name);
    ...
    let opts = HostOptions {
        ...
        project_diagnostics,
        held_symbol: held_symbol.map(str::to_string),
        ..HostOptions::default()
    };
    let result = host
...
mod conformance_generated {
    use super::{run_inline_case, run_inline_case_with, run_legacy_case_as_host, run_query_case};
