//! v1.1 manifest-metadata smoke tests.
//!
//! The diagnostics here originate in `src/ManifestLoader.zig` and
//! travel out over the JSON envelope from `sjon.wasm`. Mirrors the
//! equivalent suites in `hosts/web/test/host-v11.test.ts` and
//! `hosts/typescript-parity/test/loader-v11.test.ts`.
...
#[test]
fn v11_manifest_fully_decorated_validates_cleanly() {
...
"#;
    ...
    for code in [
        ...
        assert!(
            ...
            "unexpected diagnostic {code} on fully-canonical v1.1 manifest"
        );
    }
}

#[test]
fn v11_manifest_non_spdx_license_emits_license_unrecognized_warning() {
    let mut host = SjonHost::load(&wasm_path(), None).unwrap();
    let r = host
        .validate_document(
...
#[test]
fn v11_manifest_too_many_keywords_emits_warning() {
    let mut host = SjonHost::load(&wasm_path(), None).unwrap();
    let r = host
        .validate_document(
...
#[test]
fn v11_manifest_sjon_ahead_of_host_emits_sjon_format_unsupported_err() {
    let mut host = SjonHost::load(&wasm_path(), None).unwrap();
    let r = host
        .validate_document(
            r#"(plugin :name x :version "1.0.0" :sjon "99.0")
"#,
            &HostOptions::default(),
        )
        .unwrap();
    let d = diag_by_code(&r.diagnostics, "sjon_format_unsupported")
        .expect("expected sjon_format_unsupported");
    assert_eq!(d.severity, Severity::Err);
}

#[test]
fn v11_manifest_malformed_wasm_sha256_emits_self_hash_malformed_err() {
    let mut host = SjonHost::load(&wasm_path(), None).unwrap();
    let r = host
        .validate_document(
