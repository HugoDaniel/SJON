// v1.1 manifest-metadata smoke tests for the Web (Node + WASM) host.
...
test('v1.1 manifest: fully-decorated (plugin …) validates cleanly', async () => {
...
test('v1.1 manifest: non-SPDX :license surfaces license_unrecognized (warning) via WASM', async () => {
  const host = await SjonHost.load(wasmPath);
...
test('v1.1 manifest: >16 :keywords surfaces too_many_keywords (warning)', async () => {
  const host = await SjonHost.load(wasmPath);
...
test('v1.1 manifest: :sjon ahead of host surfaces sjon_format_unsupported (err)', async () => {
  const host = await SjonHost.load(wasmPath);
  const src = `(plugin :name x :version "1.0.0" :sjon "99.0")\n`;
  const r = host.validateDocument(src, { projectRoot: null, projectFile: null });
  const d = r.diagnostics.find((x) => x.code === 'sjon_format_unsupported');
  assert.ok(d, `expected sjon_format_unsupported in ${JSON.stringify(codes(r.diagnostics))}`);
  assert.equal(d.severity, 'err');
});

test('v1.1 manifest: malformed :wasm-sha256 surfaces plugin_wasm_self_hash_malformed (err)', async () => {
  const host = await SjonHost.load(wasmPath);
