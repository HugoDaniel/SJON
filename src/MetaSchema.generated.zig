...
const value_kinds = [_]Plugin.ValueKind{
    ...
    .{ .name = "form-child-decl", .underlying = .form, .description = "A (key …) | (variant …) | (exclusive-group …) | (form …) sub-form inside (form …).", .heads = .{ .heads = &.{ .{ .name = "key" }, .{ .name = "variant" }, .{ .name = "exclusive-group" }, .{ .name = "form" } } } },
    .{ .name = "key-local-decl", .underlying = .form, .description = "An inline slot-local (form …) sub-form inside (key …).", .heads = .{ .heads = &.{.{ .name = "form" }} } },
    .{ .name = "alt-decl", .underlying = .form, .description = "An (alt …) sub-form inside (exclusive-group …).", .heads = .{ .heads = &.{.{ .name = "alt" }} } },
    .{ .name = "signature-spec", .underlying = .form, .description = "A (signature …) sub-form for multi-signature expr-func.", .heads = .{ .heads = &.{.{ .name = "signature" }} } },
    .{ .name = "arity-spec", .underlying = .form, .description = "Arity declaration: (fixed N) | (at-least N) | (range :min M :max N).", .heads = .{ .heads = &.{ .{ .name = "fixed" }, .{ .name = "at-least" }, .{ .name = "range" } } } },
    .{ .name = "vector-shape-ref", .underlying = .form, .description = "Slot pinned to a (vector-shape …) form.", .heads = .{ .heads = &.{.{ .name = "vector-shape" }} } },
    .{ .name = "unit-shape-ref", .underlying = .form, .description = "Slot pinned to a (unit-shape …) form.", .heads = .{ .heads = &.{.{ .name = "unit-shape" }} } },
    .{ .name = "numeric-bounds-ref", .underlying = .form, .description = "Slot pinned to a (numeric-bounds …) form.", .heads = .{ .heads = &.{.{ .name = "numeric-bounds" }} } },
    .{ .name = "string-bounds-ref", .underlying = .form, .description = "Slot pinned to a (string-bounds …) form.", .heads = .{ .heads = &.{.{ .name = "string-bounds" }} } },
    ...
    .{ .name = "repr-shape-ref", .underlying = .form, .description = "Slot pinned to a (repr-shape …) form.", .heads = .{ .heads = &.{.{ .name = "repr-shape" }} } },
    ...
    .{ .name = "scalar-or-ref-shape-ref", .underlying = .form, .description = "Slot pinned to a (scalar-or-ref-shape …) form.", .heads = .{ .heads = &.{.{ .name = "scalar-or-ref-shape" }} } },
    .{ .name = "member-set-ref", .underlying = .form, .description = "Slot pinned to a (member-set …) form.", .heads = .{ .heads = &.{.{ .name = "member-set" }} } },
    .{ .name = "member-decl", .underlying = .form, .description = "A (member …) sub-form inside (member-set …).", .heads = .{ .heads = &.{.{ .name = "member" }} } },
    .{ .name = "head-set-ref", .underlying = .form, .description = "Slot pinned to a (head-set …) form.", .heads = .{ .heads = &.{.{ .name = "head-set" }} } },
    .{ .name = "head-decl", .underlying = .form, .description = "A (head …) sub-form inside (head-set …).", .heads = .{ .heads = &.{.{ .name = "head" }} } },
    .{ .name = "cross-ref-shape-ref", .underlying = .form, .description = "Slot pinned to a (cross-ref …) form.", .heads = .{ .heads = &.{.{ .name = "cross-ref" }} } },
    .{ .name = "union-shape-ref", .underlying = .form, .description = "Slot pinned to a (union-shape …) form.", .heads = .{ .heads = &.{.{ .name = "union-shape" }} } },
    .{ .name = "lowering-ref", .underlying = .form, .description = "Slot pinned to a (lowering …) form.", .heads = .{ .heads = &.{.{ .name = "lowering" }} } },
    .{ .name = "flag-decl", .underlying = .form, .description = "A (flag …) sub-form inside (flag-set …).", .heads = .{ .heads = &.{.{ .name = "flag" }} } },
    .{ .name = "flag-set-ref", .underlying = .form, .description = "Slot pinned to a (flag-set …) form.", .heads = .{ .heads = &.{.{ .name = "flag-set" }} } },
...
const forms = [_]Plugin.FormSpec{
    ...
    .{ .name = "numeric-bounds", .description = "Range / integrality / divisibility constraints on a :number underlying.", .keys = &.{
        ...
        .{ .name = "multiple-of", .value_type = .number },
    } },
    ...
    .{ .name = "scalar-or-ref-shape", .description = "Shorthand for `union [<base> <ref>]` — a scalar kind or a symbolic reference. Desugars at load time. `:ref` defaults to the primitive `symbol` (an unchecked name); naming a cross-ref kind there makes the reference half checked.", .keys = &.{
        .{ .name = "base", .value_type = .{ .named = .{ .name = "type-ref" } }, .optional = false },
        .{ .name = "ref", .value_type = .{ .named = .{ .name = "type-ref" } } },
    } },
    .{ .name = "member-set", .description = "Closed-set membership for :symbol or :string underlyings.", .positional = .{ .kind = .{ .name = "member-decl" } }, .keys = &.{
        .{ .name = "values", .value_type = .{ .named = .{ .name = "member-name-list" } } },
    } },
    .{ .name = "member", .description = "One entry in a (member-set …). Carries optional editor metadata.", .keys = &.{
        .{ .name = "name", .value_type = .{ .named = .{ .name = "member-name" } }, .optional = false },
    ...
    .{ .name = "cross-ref", .description = "Document-spanning name-reference constraint for :symbol underlyings.", .keys = &.{
        .{ .name = "target", .value_type = .{ .named = .{ .name = "form-target" } }, .optional = false },
    ...
    .{ .name = "key", .description = "One declared :keyword value slot on a form.", .positional = .{ .kind = .{ .name = "key-local-decl" } }, .keys = &.{
        ...
        .{ .name = "requires", .value_type = .{ .named = .{ .name = "symbol-list" } } },
        .{ .name = "description", .value_type = .string },
    } },
